...
+(intptr_t)createPublisherHandle :(intptr_t)nodeHandle :(Class)messageType :(NSString *)topic;
+(intptr_t)createSubscriptionHandle :(intptr_t)nodeHandle :(Class)messageType :(NSString *)topic;
+(intptr_t)createServiceHandle :(intptr_t)nodeHandle :(Class)serviceType :(NSString *)serviceName;
...
+(intptr_t)createPublisherHandle:(intptr_t)nodeHandle :(Class)messageType :(NSString *)topic {
  ...
  rosidl_message_type_support_t * ts = (rosidl_message_type_support_t *)typesupportHandle;

  rcl_ret_t ret = rcl_publisher_init(publisher, node, ts, topic_tmp, &publisher_ops);
...
+(intptr_t)createSubscriptionHandle:(intptr_t)nodeHandle :(Class)messageType :(NSString *)topic {
  ...
  rcl_ret_t ret = rcl_subscription_init(subscription, node, ts, topic_tmp, &subscription_ops);
...
+(intptr_t)createServiceHandle:(intptr_t)nodeHandle :(Class)serviceType :(NSString *)serviceName {
  ...
  rosidl_service_type_support_t * ts = (rosidl_service_type_support_t *)serviceTypesupportHandle;

  rcl_ret_t ret = rcl_service_init(service, node, ts, serviceName_tmp, &service_ops);
...
+(intptr_t)createClientHandle:(intptr_t)nodeHandle :(Class)serviceType :(NSString *)serviceName {
  intptr_t serviceTypesupportHandle = [serviceType serviceTypesupportHandle];
  ...
  rcl_ret_t ret = rcl_client_init(client, node, ts, serviceName_tmp, &client_ops);
...
-(ROSPublisher *)createPublisher: (Class)messageType :(NSString *)topic {
  intptr_t publisherHandle = [ROSNode createPublisherHandle :self.nodeHandle : messageType : topic];
...
-(ROSSubscription *)createSubscriptionWithCallback :(Class)messageType :(NSString *)topic :(void(^)(id))callback {
  intptr_t subscriptionHandle = [ROSNode createSubscriptionHandle :self.nodeHandle :messageType :topic];
...
-(ROSService *)createServiceWithCallback :(Class)serviceType :(NSString *)serviceName :(void(^)(id, id, id))callback {
  intptr_t serviceHandle = [ROSNode createServiceHandle :self.nodeHandle :serviceType :serviceName];
...
-(ROSClient *)createClient :(Class)serviceType :(NSString *)serviceName {
  intptr_t clientHandle = [ROSNode createClientHandle :self.nodeHandle :serviceType :serviceName];
  ROSClient *client = [[ROSClient alloc] initWithArguments :self.nodeHandle :clientHandle :serviceType :serviceName];
  [[self clients] addObject :client];
  return client;
}

-(id)initWithNameAndHandle:(NSString *)nodeName :(intptr_t)nodeHandle {
